...
/// <summary>
/// Holds the partitions this broker serves, recovering each the first time it is
/// touched.
/// </summary>
/// <remarks>
/// Dead-letter queues are ordinary entities (ADR 0006) but have no row of their own:
/// their configuration is derived from the entity they belong to. That keeps
/// "create a queue" a single atomic write while still giving the dead-letter queue a
/// real log, cursor and delivery state.
/// </remarks>
...
{
    private readonly ConcurrentDictionary<string, Lazy<Task<PartitionProcessor[]>>> _partitions = new(StringComparer.Ordinal);
    ...
    public async Task<PartitionProcessor[]> GetAsync(EntityPath path, CancellationToken cancellationToken = default)
    {
        var lazy = _partitions.GetOrAdd(
            path.Value,
            key => new Lazy<Task<PartitionProcessor[]>>(() => CreateAsync(path, CancellationToken.None)));

        return await lazy.Value.WaitAsync(cancellationToken);
    }

    public async Task<PartitionProcessor> GetAsync(
        EntityPath path,
        int partitionId,
        CancellationToken cancellationToken = default)
    {
        ...
        if (partitionId < 0 || partitionId >= partitions.Length)
        {
            throw DistMqException.Invalid(
                $"Partition {partitionId} is out of range for '{path.Value}', which has {partitions.Length}.");
        }
    ...
    private async Task<PartitionProcessor[]> CreateAsync(EntityPath path, CancellationToken cancellationToken)
    {
        ...
        for (var partitionId = 0; partitionId < processors.Length; partitionId++)
        {
            ...
            processors[partitionId] = new PartitionProcessor(
                descriptor, partitionId, log, objects, DeadLetterAsync, _time);

            await processors[partitionId].RecoverAsync(cancellationToken);
        }
